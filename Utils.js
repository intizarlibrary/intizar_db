...
function exportData(type, user) {
  ...
  if (data.length <= 1) {
    throw new Error('No data to export. The ' + sheetName + ' sheet is empty.');
  }
  ...
  logAudit('Admin', 'EXPORT_' + type.toUpperCase(), 'Exported ' + sheetName + ' data (' + (data.length - 1) + ' records)');
...
// ==================== GET SINGLE MEMBER (Admin can view archived) ====================
function getMember(intizarId, user) {
  ...
  for (let i = 1; i < data.length; i++) {
    if (data[i][0] === intizarId) {
      // Only block if not Admin AND not active
...
// ==================== UPDATE MEMBER (Admin only) – FIXED ====================
